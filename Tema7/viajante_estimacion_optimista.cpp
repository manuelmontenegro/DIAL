...
void viajante(const grafo<int> &g, int min_arista,
              int k, vector<int> &sol, int coste_acum,
  ...
  if (k == sol.size()) {
    ...
    for (int c = 0; c < num_vertices(g); c++) {
      if (es_factible(g, k, c, sol, visitadas)) {
        sol[k] = c;
        int coste_optimista = coste_acum + g[sol[k - 1]][c] + (num_vertices(g) - k) * min_arista;
...
// Llamada inicial a la función de vuelta atrás
// --------------------------------------------
